...
async function initCurrentBudget(
  userDocSnap: DocumentSnapshot,
  userId: string,
  userDoc: DocumentReference
) {
  const initialBudget: { cash: number; account: number } = userDocSnap.get(
    'initialBudget'
  );
  const itemCollection: CollectionReference = db.collection(
    `/users/${userId}/items`
  );
  const itemDocRefs: DocumentReference[] = await itemCollection.listDocuments();
  const itemDocPromises: Promise<DocumentSnapshot>[] = itemDocRefs.map((doc) =>
    doc.get()
  );
  ...
  const cashSum: number = items
    .filter((item) => item.origin === 'cash')
    .reduce(reducer, 0);
  const accountSum: number = items
    .filter((item) => item.origin === 'account')
    .reduce(reducer, 0);
...
function updateCurrentBudget(
  currentBudget: { cash: number; account: number },
  ...
  if (!change.after.exists) {
    value = change.before.get('value') * -1;
  } // document deleted
  if (change.after.exists && change.before.exists) {
    value = change.after.get('value') - change.before.get('value');
  } // document updated
...
async function updateBudget(
  ...
  const currentBudget: { cash: number; account: number } = userDocSnap.get(
    'currentBudget'
  );
...
exports.updateBudget = functions.firestore
  .document('/users/{userId}/items/{itemId}')
  .onWrite(updateBudget);
