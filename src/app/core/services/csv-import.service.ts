...
export class CsvImportService {
    ...
    this.getExistingItems(uid, items)
      .pipe(
        map((existingItems) => {
          if (existingItems.length > 0) {
            return items.filter(
              (item) =>
                !existingItems.some(
                  (existingItem) => existingItem.importId === item.importId
                )
            );
          }
          return items;
        ...
        tap((results: any[]) =>
          this.snackBar.open(
            `${results.length} ${
              results.length > 1 ? 'Einträge' : 'Eintrag'
  ...
  private parseSparkasseExport(result: ParseResult<any>): Item[] {
    ...
    const receiverIndex: number = header.indexOf(
  ...
  private createExistingItemQuery(items: Item[]) {
    ...
    const endDate: number = items.reduce(
      (max: number, cur: Item) => Math.max(max, cur.date.toMillis()),
      startDate
    );
